...
async def notify_admins(
    ...
    text: str | None = None,
    media_files: list | None = None,  # 🔥 Новий аргумент: список файлів
    ...
    # Формуємо текст повідомлення (інфо про юзера + контент)
    if is_anonymous:
    ...
    for admin_id in settings.ADMIN_IDS:
        try:
            ...
            for m in media_files:
                ...
                # Документи в альбомах Telegram часто не підтримуються разом з фото/відео, 
                # тому тут обробляємо тільки фото/відео для прев'ю.

            if media_group:
                # Відправляємо альбом (без кнопок)
                await bot.send_media_group(admin_id, media=media_group)
            
            # 2. Відправляємо окреме повідомлення з текстом і кнопками ("Пульт керування")
